...
The selectable log keeps the latest 65,536 characters and marks truncation. Information and `Write-Host` messages appear without a stream prefix; other records retain their stream label. Consecutive blank lines are collapsed to one throughout the log. It preserves order within each stream, not between streams. Success objects use plain-text PowerShell formatting. Direct console writes and independently launched process output are not captured.
