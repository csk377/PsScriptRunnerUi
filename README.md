...
The demo simulates success, confirmation, failure, cancellation, and a progress flood. Declining confirmation prevents launch.
...
`ProgressRecordsRead` counts every drained record, including updates skipped for display. Before allowing the dialog to close, the module calls `EndInvoke`, drains remaining progress and information records, and displays the result. Idle polls drain information without redrawing progress.
...
The result contains `Status`, `Duration`, `CancellationWasRequested`, `ErrorRecord`, and `ProgressRecordsRead`:

- An error, including nonterminating `Write-Error`, produces `Failed`.
...
Scripts must check native exit codes and convert failures into errors. `-CloseOnSuccess` closes only `Completed` results. Otherwise, click Close.

The result panel shows a selectable message, green for success, yellow for cancellation, or red for failure. Normal completion after a cancellation request stays green and mentions the request. Error messages prefer explicit error details over the exception message and omit the `EndInvoke` wrapper. The result keeps the full `ErrorRecord`.
